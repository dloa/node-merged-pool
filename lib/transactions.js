...
function getTXComment(txMessages, callback) {
    if (txMessages)
        request({
            ...
            if (err) return console.error(err);
                ...
                callback(util.serializeString(JSON.stringify({
                    blocks: body.blocks,
                    difficulty: body.difficulty,
                    networkhashps: body.networkhashps
                })));
        });
    else
        callback(new Buffer([]));
...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree, callback) {
    ...
    getTXComment(txMessages, function(txComment) {
