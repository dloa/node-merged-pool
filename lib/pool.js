    ...
    function updateDloaComment() {
        async.parallel({
                ...
                fmd: function (callback) {
                    request('http://libraryd.alexandria.media:41290/flo-market-data/v1/getAll', function (error, response, body) {
                        if (!error && response.statusCode == 200) {
                            try {
                                var parsed = JSON.parse(body);
                                return callback(null, {
                ...
                fbd: function (callback) {
                    request('http://hashreport.alexandria.media:5831/getMiningInfo', function (error, response, body) {
                        if (!error && response.statusCode == 200) {
                            try {
                                var parsed = JSON.parse(body);
                                return callback(null, {
            ...
            function (err, results) {
                if (!err) {
                    var api_arr = ['pool.alexandria.media',
                        results.mrr.last_10,
                    ...
                    dloaCommentCache = 'pool.alexandria.media:error';
            });
    }
