...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    var dloaCommentCache = 'pool.alexandria.media';
    ...
    function updateDloaComment() {
        async.parallel({
                ...
                pool: function (callback) {
                    request('http://pool.alexandria.io/api/stats', function (error, response, body) {
                        if (!error && response.statusCode == 200) {
                            try {
                                var parsed = JSON.parse(body);
                                return callback(null, {
                ...
                fmd: function (callback) {
                    request('http://libraryd.alexandria.io:41290/flo-market-data/v1/getAll', function (error, response, body) {
                        if (!error && response.statusCode == 200) {
                            try {
                                var parsed = JSON.parse(body);
                                return callback(null, {
                ...
                fbd: function (callback) {
                    request('http://hashreport.alexandria.io:5831/getMiningInfo', function (error, response, body) {
                        if (!error && response.statusCode == 200) {
                            try {
                                var parsed = JSON.parse(body);
                                return callback(null, {
            ...
            function (err, results) {
                if (!err) {
                    var api_arr = ['pool.alexandria.io',
        ...
        _this.daemon.cmd('getblocktemplate',
            ...
            function(result){
                if (result.error){
                    ...
                    data.dloaComment = dloaCommentCache;
                    data.auxes = [];
